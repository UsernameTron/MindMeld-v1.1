...
def check_remaining_tool_issues():
    ...
    try:
        ...
        double_quotes_cmd = (
            'grep -r --include="*.py" -E \'"type"\\s*:\\s*"function"\' packages/agents/'
        )
        # Check single quotes syntax
        single_quotes_cmd = "grep -r --include=\"*.py\" -E \"'type'\\s*:\\s*'function'\" packages/agents/"

        # Run both commands
        double_quotes_result = subprocess.run(
            double_quotes_cmd, shell=True, capture_output=True, text=True
        )
        single_quotes_result = subprocess.run(
            single_quotes_cmd, shell=True, capture_output=True, text=True
        )
