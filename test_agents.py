...
from packages.agents.advanced_reasoning.agents import TestGeneratorAgent, DependencyAgent

def test_test_generator_agent():
    with patch('ollama.Client') as mock_client:
        mock_instance = MagicMock()
        mock_instance.chat.return_value.message.content = "def test_example(): assert True"
...
def test_dependency_agent():
    with patch('os.walk') as mock_walk, patch('builtins.open', create=True), patch('ast.parse'):
