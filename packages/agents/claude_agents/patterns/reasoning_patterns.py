...
class SelfReflectionPattern(ReasoningPattern):
    ...
    def parse_response(self, response: str) -> Dict[str, Any]:
        ...
        refl_match = re.search(
            r"(?:^|\n)(?:Reflection:|Self-critique:|Evaluation:)(.*?)(?=\n(?:Refined solution:|Improved answer:|Final solution:))",
            response,
            re.IGNORECASE | re.DOTALL,
        )
        if refl_match:
...
class DebatePattern(ReasoningPattern):
    ...
    def parse_response(self, response: str) -> Dict[str, Any]:
        ...
        perspective_matches = re.findall(
            perspective_pattern, response, re.IGNORECASE | re.DOTALL
        )
        ...
        synth_match = re.search(
            r"(?:^|\n)(?:Synthesis:|Conclusion:|Balance:|Integrated view:)(.*?)$",
            response,
            re.IGNORECASE | re.DOTALL,
        )
        if synth_match:
...
class FirstPrinciplesPattern(ReasoningPattern):
    ...
    def parse_response(self, response: str) -> Dict[str, Any]:
        ...
        solution = ""
        sol_match = re.search(
            r"(?:^|\n)(?:4\.|Rebuild:|Solution construction:|Building from fundamentals:)(.*?)(?=(?:\n(?:5\.|Verify:|Verification:|Solution check:))|\Z)",
            response,
            re.IGNORECASE | re.DOTALL,
        )
        if sol_match:
        ...
        verification = ""
        ver_match = re.search(
            r"(?:^|\n)(?:5\.|Verify:|Verification:|Solution check:)(.*?)$",
            response,
            re.IGNORECASE | re.DOTALL,
        )
        if ver_match:
...
class ReasoningPatternLibrary:
    ...
    def recommend_pattern(self, task: str, domain: str) -> Dict[str, Any]:
        ...
        if not any(scores.values()):
            ...
            matching_domains = [
                d
                for d, patterns in domain_affinities.items()
                if d in domain_lower and top_pattern in patterns
            ]

            reasoning_parts = []
            if matching_keywords:
                reasoning_parts.append(
                    f"Task contains relevant keywords: {', '.join(matching_keywords)}."
                )
            if matching_domains:
                reasoning_parts.append(
                    f"Task domain ({domain}) has affinity with this pattern."
                )

            reasoning = (
                " ".join(reasoning_parts)
                if reasoning_parts
                else "Best overall match for the given task and domain."
            )
    ...
    def apply_pattern(
        self, pattern_name: str, task: str, agent_type: str = "planner", **kwargs
    ) -> Dict[str, str]:
        """
...
DOMAIN_PRESETS = {
    ...
    "scientific_research": {"pattern": "scientific_method", "domain": "research"},
    "business_strategy": {"pattern": "first_principles", "domain": "business"},
    "content_creation": {
        "pattern": "self_reflection",
        "domain": "writing",
        "iterations": 2,
    },
    "engineering_design": {"pattern": "first_principles", "domain": "engineering"},
    "policy_analysis": {
        "pattern": "debate",
        "domain": "policy",
