...
class AgentMonitor:
    ...
    def track_api_call(
        ...
        try:
            ...
            self.metrics["system"]["cpu_usage_percent"] = process.cpu_percent(interval=0.1)
        except ImportError:
            pass
    ...
    def track_agent_invocation(
        ...
        if agent_metrics["invocations"] == 1:
            ...
            agent_metrics["avg_response_time"] = (
                agent_metrics["avg_response_time"] * (agent_metrics["invocations"] - 1) + response_time
    ...
    def track_pipeline_task(
        ...
        total_tasks = self.metrics["pipeline"]["tasks_completed"] + self.metrics["pipeline"]["tasks_failed"]
        ...
        if total_tasks == 1:
            ...
            self.metrics["pipeline"]["avg_iterations"] = (
                self.metrics["pipeline"]["avg_iterations"] * (total_tasks - 1) + iterations
            ) / total_tasks
            
            self.metrics["pipeline"]["avg_completion_time"] = (
                self.metrics["pipeline"]["avg_completion_time"] * (total_tasks - 1) + completion_time
            ) / total_tasks
        
        # Update token efficiency
        if self.metrics["pipeline"]["tasks_completed"] > 0:
            self.metrics["pipeline"]["token_efficiency"] = (
                self.metrics["api"]["total_tokens"] / self.metrics["pipeline"]["tasks_completed"]
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        if self.metrics["api"]["total_requests"] > 0:
            api_success_rate = self.metrics["api"]["successful_requests"] / self.metrics["api"]["total_requests"]
        
        pipeline_success_rate = 0
        total_tasks = self.metrics["pipeline"]["tasks_completed"] + self.metrics["pipeline"]["tasks_failed"]
        if total_tasks > 0:
            pipeline_success_rate = self.metrics["pipeline"]["tasks_completed"] / total_tasks
        
        # Calculate average response time
        avg_response_time = 0
        if self.metrics["api"]["response_times"]:
            avg_response_time = sum(self.metrics["api"]["response_times"]) / len(self.metrics["api"]["response_times"])
    ...
    def _load_historical_metrics(self) -> None:
        ...
        if os.path.exists(self.metrics_file):
            try:
                ...
                logger.info(f"Loaded {len(self.historical_metrics)} historical metrics entries")
    ...
    def _cleanup_old_metrics(self) -> None:
        ...
        if original_count < len(self.historical_metrics):
            ...
            logger.info(f"Cleaned up {original_count - len(self.historical_metrics)} old metrics entries")
    ...
    def _setup_dashboard(self) -> None:
        ...
        if not DASH_AVAILABLE:
            logger.warning("Dash not available. Install dash, pandas, and plotly to enable the web dashboard.")
            return
        ...
        app.layout = html.Div([
            ...
            dcc.Tabs([
                ...
                # API metrics tab
                dcc.Tab(label="API Metrics", children=[
                    html.Div([
                        html.H2("API Usage"),
                ...
                # Agent metrics tab
                dcc.Tab(label="Agent Metrics", children=[
                    html.Div([
                ...
                dcc.Tab(label="Pipeline Metrics", children=[
                    html.Div([
        ...
        def update_overview_stats(n):
            ...
            return html.Div([
                ...
                html.P(f"Tasks completed: {metrics['pipeline']['tasks_completed']}"),
                html.P(f"Memory usage: {metrics['system']['memory_usage_mb']:.1f} MB"),
            ])
        ...
        def update_cost_projection(n):
            ...
            budget_status = "Within budget" if projection["within_budget"] else "Exceeding budget"
            ...
            return html.Div([
                ...
                html.P(f"Budget status: {budget_status}", 
                       style={"color": "green" if projection["within_budget"] else "red"}),
            ])
        ...
        def update_api_stats(n):
            ...
            return html.Div([
                ...
                html.P(f"Successful requests: {api_metrics['successful_requests']}"),
        ...
        def update_api_usage_chart(n):
            ...
            fig = px.line(
                df, x="timestamp", y=["total_requests", "successful_requests", "failed_requests"],
        ...
        def update_agent_stats(n):
            ...
            for agent_type, agent_data in agent_metrics.items():
                ...
                if agent_data["invocations"] > 0:
                    success_rate = agent_data["successful"] / agent_data["invocations"] * 100
                
                agent_stats.append(html.Div([
                    html.H4(f"{agent_type.capitalize()} Agent"),
                    html.P(f"Invocations: {agent_data['invocations']}"),
                    html.P(f"Success rate: {success_rate:.1f}%"),
                    html.P(f"Avg response time: {agent_data['avg_response_time']:.2f} sec"),
        ...
        def update_agent_performance_chart(n):
            ...
            fig.add_trace(go.Bar(
                x=df["agent"], y=df["success_rate"],
                name="Success Rate (%)",
                marker_color="green"
            ))
            
            fig.add_trace(go.Bar(
                x=df["agent"], y=df["avg_response_time"],
        ...
        def update_pipeline_stats(n):
            ...
            success_rate = 0
            total_tasks = pipeline_metrics["tasks_completed"] + pipeline_metrics["tasks_failed"]
            if total_tasks > 0:
            ...
            return html.Div([
                ...
                html.P(f"Success rate: {success_rate:.1f}%"),
                html.P(f"Avg iterations per task: {pipeline_metrics['avg_iterations']:.2f}"),
                html.P(f"Avg completion time: {pipeline_metrics['avg_completion_time']:.2f} sec"),
                html.P(f"Token efficiency: {pipeline_metrics['token_efficiency']:.1f} tokens/task"),
            ])
        ...
        def update_pipeline_performance_chart(n):
            ...
            fig.add_trace(
                go.Scatter(
                    x=df["timestamp"], y=df["success_rate"],
                    name="Success Rate (%)",
            ...
            fig.add_trace(
                go.Scatter(
                    x=df["timestamp"], y=df["avg_completion_time"],
        ...
        logger.info(f"Monitoring dashboard running at http://localhost:{self.dashboard_port}")
        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if isinstance(result, dict):
                    ...
                    if "final_results" in result and "step_results" in result["final_results"]:
                        for step_result in result["final_results"]["step_results"].values():
                            if isinstance(step_result, dict) and "tokens" in step_result:
