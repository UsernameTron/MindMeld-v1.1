...
class PrometheusExporter(MetricsExporter):
    ...
    def __init__(self, agent_monitor, port: int = 8000, endpoint: str = "/metrics"):
        """
        ...
        try:
            ...
            self.api_cost = Counter(
                "claude_agents_api_cost_total", "Total API cost in USD"
            )
            ...
            self.system_uptime = Gauge(
                "claude_agents_system_uptime_seconds", "System uptime in seconds"
            )
            self.system_memory_usage = Gauge(
                "claude_agents_system_memory_usage_mb", "Memory usage in MB"
            )

            # Start HTTP server
            start_http_server(port)
            logger.info(
                f"Prometheus metrics server running on port {port} at endpoint {endpoint}"
            )

        except ImportError:
            logger.warning(
                "prometheus_client not available. Install prometheus_client to export metrics to Prometheus."
            )
    ...
    def export_metrics(self):
        ...
        self.api_requests.labels(status="success").inc(
            metrics["api"]["successful_requests"]
        )
        self.api_requests.labels(status="failure").inc(
            metrics["api"]["failed_requests"]
        )
        self.api_tokens.labels(type="prompt").inc(metrics["api"]["prompt_tokens"])
        self.api_tokens.labels(type="completion").inc(
            metrics["api"]["completion_tokens"]
        )
        self.api_cost.inc(metrics["api"]["total_cost"])

        # Update agent metrics
        for agent_type, agent_data in metrics["agents"].items():
            self.agent_invocations.labels(agent=agent_type, status="success").inc(
                agent_data["successful"]
            )
            self.agent_invocations.labels(agent=agent_type, status="failure").inc(
                agent_data["failed"]
            )
            self.agent_response_time.labels(agent=agent_type).set(
                agent_data["avg_response_time"]
            )
            self.agent_cost.labels(agent=agent_type).inc(agent_data["total_cost"])

        # Update pipeline metrics
        self.pipeline_tasks.labels(status="completed").inc(
            metrics["pipeline"]["tasks_completed"]
        )
        self.pipeline_tasks.labels(status="failed").inc(
...
class DatadogExporter(MetricsExporter):
    ...
    def __init__(
        ...
        try:
            ...
            if not self.api_key:
                logger.warning(
                    "Datadog API key not provided. Some features may not work."
                )
            ...
            logger.warning(
                "datadog not available. Install datadog to export metrics to Datadog."
            )
    ...
    def export_metrics(self):
        ...
        # Send API metrics
        self.statsd.gauge(
            "claude_agents.api.requests.total",
            metrics["api"]["total_requests"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.api.requests.successful",
            metrics["api"]["successful_requests"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.api.requests.failed",
            metrics["api"]["failed_requests"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.api.tokens.prompt",
            metrics["api"]["prompt_tokens"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.api.tokens.completion",
            metrics["api"]["completion_tokens"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.api.cost", metrics["api"]["total_cost"], tags=self.tags
        )
        ...
        for agent_type, agent_data in metrics["agents"].items():
            ...
            self.statsd.gauge(
                "claude_agents.agent.invocations",
                agent_data["invocations"],
                tags=agent_tags,
            )
            self.statsd.gauge(
                "claude_agents.agent.successful",
                agent_data["successful"],
                tags=agent_tags,
            )
            self.statsd.gauge(
                "claude_agents.agent.failed", agent_data["failed"], tags=agent_tags
            )
            self.statsd.gauge(
                "claude_agents.agent.response_time",
                agent_data["avg_response_time"],
                tags=agent_tags,
            )
            self.statsd.gauge(
                "claude_agents.agent.tokens",
                agent_data["total_tokens"],
                tags=agent_tags,
            )
            self.statsd.gauge(
                "claude_agents.agent.cost", agent_data["total_cost"], tags=agent_tags
            )

        # Send pipeline metrics
        self.statsd.gauge(
            "claude_agents.pipeline.tasks.completed",
            metrics["pipeline"]["tasks_completed"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.pipeline.tasks.failed",
            metrics["pipeline"]["tasks_failed"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.pipeline.iterations",
            metrics["pipeline"]["avg_iterations"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.pipeline.completion_time",
            metrics["pipeline"]["avg_completion_time"],
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.pipeline.token_efficiency",
            metrics["pipeline"]["token_efficiency"],
            tags=self.tags,
        )

        # Send system metrics
        self.statsd.gauge(
            "claude_agents.system.uptime", metrics["system"]["uptime"], tags=self.tags
        )
        self.statsd.gauge(
            "claude_agents.system.memory",
        ...
        self.statsd.gauge(
            "claude_agents.performance.api_success_rate",
            perf_metrics["api_success_rate"] * 100,
            tags=self.tags,
        )
        self.statsd.gauge(
            "claude_agents.performance.pipeline_success_rate",
            perf_metrics["pipeline_success_rate"] * 100,
            tags=self.tags,
        )
        ...
        self.statsd.gauge(
            "claude_agents.cost.hourly", cost_projection["hourly_cost"], tags=self.tags
        )
        self.statsd.gauge(
            "claude_agents.cost.daily", cost_projection["daily_cost"], tags=self.tags
        )
        self.statsd.gauge(
            "claude_agents.cost.monthly",
...
class SlackAlerter(MetricsExporter):
    ...
    def __init__(
        ...
        self.thresholds = thresholds or {
            ...
            "system": {"memory_mb": 1000},  # Alert when memory usage exceeds 1000 MB
        }
    ...
    def export_metrics(self):
        ...
        if (
            cost_projection["monthly_cost"]
            > self.thresholds["cost"]["monthly_projection"]
        ):
            self._send_alert(
                "cost_monthly",
        ...
        if (
            perf_metrics["api_success_rate"]
            < self.thresholds["performance"]["api_success_rate"]
        ):
            self._send_alert(
                "api_success_rate",
                f"🔴 API success rate of {perf_metrics['api_success_rate']*100:.1f}% below threshold of {self.thresholds['performance']['api_success_rate']*100:.1f}%",
                {
                    ...
                    "Common Errors": self._format_errors(
                        metrics["api"]["error_counts"]
                    ),
                },
            )

        if (
            perf_metrics["pipeline_success_rate"]
            < self.thresholds["performance"]["pipeline_success_rate"]
        ):
            self._send_alert(
        ...
        if (
            metrics["system"]["memory_usage_mb"]
            > self.thresholds["system"]["memory_mb"]
        ):
            self._send_alert(
                "memory_usage",
    ...
    def _send_alert(self, alert_type: str, message: str, fields: Dict[str, str]):
        ...
        if alert_type in self.last_alert_times:
            ...
            if time_since_last < self.alert_cooldown:
                logger.debug(
                    f"Skipping {alert_type} alert due to cooldown ({time_since_last:.1f}s < {self.alert_cooldown}s)"
                )
                return
        ...
        for name, value in fields.items():
            formatted_fields.append({"title": name, "value": value, "short": True})
...
class NewRelicExporter(MetricsExporter):
    ...
    def __init__(
        ...
        try:
            ...
            if not self.api_key:
                logger.warning(
                    "New Relic API key not provided. Some features may not work."
                )
            ...
            logger.warning(
                "newrelic not available. Install newrelic to export metrics to New Relic."
            )
    ...
    def export_metrics(self):
        ...
        # API metrics
        self.newrelic.record_custom_metric(
            "Custom/API/Requests/Total", metrics["api"]["total_requests"]
        )
        self.newrelic.record_custom_metric(
            "Custom/API/Requests/Successful", metrics["api"]["successful_requests"]
        )
        self.newrelic.record_custom_metric(
            "Custom/API/Requests/Failed", metrics["api"]["failed_requests"]
        )
        self.newrelic.record_custom_metric(
            "Custom/API/Tokens/Prompt", metrics["api"]["prompt_tokens"]
        )
        self.newrelic.record_custom_metric(
            "Custom/API/Tokens/Completion", metrics["api"]["completion_tokens"]
        )
        self.newrelic.record_custom_metric(
            "Custom/API/Cost/Total", metrics["api"]["total_cost"]
        )

        # Agent metrics
        for agent_type, agent_data in metrics["agents"].items():
            self.newrelic.record_custom_metric(
                f"Custom/Agent/{agent_type}/Invocations", agent_data["invocations"]
            )
            self.newrelic.record_custom_metric(
                f"Custom/Agent/{agent_type}/Successful", agent_data["successful"]
            )
            self.newrelic.record_custom_metric(
                f"Custom/Agent/{agent_type}/Failed", agent_data["failed"]
            )
            self.newrelic.record_custom_metric(
                f"Custom/Agent/{agent_type}/ResponseTime",
                agent_data["avg_response_time"],
            )
            self.newrelic.record_custom_metric(
                f"Custom/Agent/{agent_type}/Tokens", agent_data["total_tokens"]
            )
            self.newrelic.record_custom_metric(
                f"Custom/Agent/{agent_type}/Cost", agent_data["total_cost"]
            )

        # Pipeline metrics
        self.newrelic.record_custom_metric(
            "Custom/Pipeline/Tasks/Completed", metrics["pipeline"]["tasks_completed"]
        )
        self.newrelic.record_custom_metric(
            "Custom/Pipeline/Tasks/Failed", metrics["pipeline"]["tasks_failed"]
        )
        self.newrelic.record_custom_metric(
            "Custom/Pipeline/Iterations", metrics["pipeline"]["avg_iterations"]
        )
        self.newrelic.record_custom_metric(
            "Custom/Pipeline/CompletionTime", metrics["pipeline"]["avg_completion_time"]
        )
        self.newrelic.record_custom_metric(
            "Custom/Pipeline/TokenEfficiency", metrics["pipeline"]["token_efficiency"]
        )

        # Performance metrics
        self.newrelic.record_custom_metric(
            "Custom/Performance/API/SuccessRate", perf_metrics["api_success_rate"]
        )
        self.newrelic.record_custom_metric(
            "Custom/Performance/Pipeline/SuccessRate",
            perf_metrics["pipeline_success_rate"],
        )

        # Cost projection
        self.newrelic.record_custom_metric(
            "Custom/Cost/Hourly", cost_projection["hourly_cost"]
        )
        self.newrelic.record_custom_metric(
            "Custom/Cost/Daily", cost_projection["daily_cost"]
        )
        self.newrelic.record_custom_metric(
            "Custom/Cost/Monthly", cost_projection["monthly_cost"]
        )

        # Add these metrics to the current transaction
        txn = self.newrelic.current_transaction()
        if txn:
            txn.add_custom_parameter(
                "api_success_rate", perf_metrics["api_success_rate"]
            )
            txn.add_custom_parameter(
                "pipeline_success_rate", perf_metrics["pipeline_success_rate"]
            )
            txn.add_custom_parameter(
                "monthly_cost_projection", cost_projection["monthly_cost"]
            )
            txn.add_custom_parameter(
