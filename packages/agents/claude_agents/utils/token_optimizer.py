...
class TokenOptimizer:
    ...
    def compress_prompt(prompt: str, compression_level: str = "medium") -> str:
        ...
        if prompt != compressed:
            ...
            logger.debug(f"Compressed prompt from {orig_len} to {new_len} chars ({reduction:.1f}% reduction)")
            
        return compressed
    
    @staticmethod
    def compress_messages(messages: List[Dict[str, Any]], compression_level: str = "medium") -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def optimize_api_parameters(params: Dict[str, Any], budget_tier: str = "standard") -> Dict[str, Any]:
        """
        ...
        if budget_tier == "economy":
            ...
            optimized["temperature"] = 0.3  # Lower temp for more deterministic (and often shorter) responses
    ...
    def optimize_for_budget(
        ...
        for model in models:
            for compression in compression_levels:
                ...
                compressed_prompt_tokens = int(avg_prompt_tokens * (1 - token_reduction[compression]))
        ...
        return {
            "original_monthly_cost": TokenOptimizer.estimate_monthly_cost(
                avg_prompt_tokens, avg_completion_tokens, requests_per_day, "claude-3-opus-20240229"
            )["total_cost"],
    ...
    def _generate_optimization_suggestions(
        ...
        if recommendation["monthly_cost"] > target_budget:
            ...
            suggestions.append(f"Switch to {recommendation['model']} with {recommendation['compression_level']} compression")
            suggestions.append("Implement response caching for common queries")
            suggestions.append("Reduce max_tokens limit further and implement chunking for long outputs")
            suggestions.append("Consider reducing daily request volume or batching similar requests")
        else:
            ...
            if headroom_percentage < 10:
                # Very tight margin
                suggestions.append("Current configuration fits within budget but with minimal margin")
                suggestions.append("Implement usage monitoring to avoid unexpected overages")
            elif "haiku" in recommendation["model"] and headroom_percentage > 30:
                # Could potentially upgrade model
                suggestions.append("Consider upgrading to claude-3-sonnet for improved quality while staying within budget")
            elif "sonnet" in recommendation["model"] and headroom_percentage > 50:
                # Could potentially upgrade model
                suggestions.append("Consider upgrading to claude-3-opus for critical reasoning tasks while staying within budget")
                
            if recommendation["compression_level"] == "high" and headroom_percentage > 20:
                suggestions.append("Could reduce compression level to improve prompt quality while staying within budget")
