    ...
    def send_message(
        ...
        while attempts < self.max_retries:
            try:
                ...
                if "server error" in str(e).lower() or "5" in str(e):
                    backoff_time = self._exponential_backoff(attempts)
                    logger.warning(
                        f"Server error. Retrying in {backoff_time} seconds..."
                    )
                    time.sleep(backoff_time)
                    continue
        ...
        # If we exit the loop without returning, raise an exception to satisfy the return type
        raise RuntimeError(
            "Failed to get a response from Claude API after maximum retries."
        )
