...
def generate_context_file():
    ...
    try:
        ...
        if factory_content:
            ...
            agent_types = re.findall(
                r"AGENT_REGISTRY\s*=\s*{([^}]*)}", factory_content, re.DOTALL
            )
            if agent_types:
                ...
                for agent in agents:
                    if ":" in agent:
                        ...
                        # Use a regular string with format() to avoid f-string backslash issue
                        name_cleaned = name.strip().strip("\"'")
                        context_content += f"- {name_cleaned}: {impl.strip()}\n"
        ...
        if app_content:
            ...
            routes = re.findall(
                r'@app\.(?:route|get|post|put|delete)\([\'"]([^\'"]*)[\'"]', app_content
            )
            if routes:
                context_content += "\n### API Endpoints\n\n"
                for route in routes:
